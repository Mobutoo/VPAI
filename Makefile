...
check: ## Dry-run du playbook principal (--check --diff)
	$(ANSIBLE_PLAYBOOK) playbooks/stacks/site.yml --check --diff
...
openclaw-oauth-start: ## Étape 1 OAuth : générer l'URL d'autorisation OpenAI
	...
	$(ANSIBLE_PLAYBOOK) playbooks/utils/openclaw-oauth.yml \
		-e "target_env=prod" \
		-e "oauth_step=start"
...
vpn-on: ## Basculer en mode VPN-only (ports fermés, webhooks via relay)
	...
	@read CONFIRM && \
	if [ "$$CONFIRM" = "yes" ]; then \
		$(ANSIBLE_PLAYBOOK) playbooks/utils/vpn-toggle.yml \
			-e "vpn_mode=on" \
			--diff; \
	else \
		echo "$(YELLOW)>>> Aborted$(NC)"; exit 1; \
...
vpn-status: ## Afficher l'état VPN actuel (UFW, Caddy, relay)
	...
	$(ANSIBLE_PLAYBOOK) playbooks/ops/safety-check.yml -e "target_env=prod"
...
backup-restore: ## Restaurer depuis un backup S3
	$(ANSIBLE_PLAYBOOK) playbooks/ops/backup-restore.yml --diff
...
rollback: ## Rollback à la version précédente
	...
	@read CONFIRM && \
	if [ "$$CONFIRM" = "yes" ]; then \
		$(ANSIBLE_PLAYBOOK) playbooks/ops/rollback.yml --diff; \
	else \
		echo "$(YELLOW)>>> Aborted$(NC)"; exit 1; \
	fi
...
rotate-secrets: ## Rotation de tous les secrets
	$(ANSIBLE_PLAYBOOK) playbooks/ops/rotate-secrets.yml --diff
...
deploy-workstation: ## Deployer la workstation Pi
	...
	$(ANSIBLE_PLAYBOOK) playbooks/hosts/workstation.yml --diff

.PHONY: deploy-memory-worker
deploy-memory-worker: ## Deployer uniquement le worker memoire sur la workstation Pi
	$(ANSIBLE_PLAYBOOK) playbooks/hosts/workstation.yml --tags "llamaindex-memory-worker" --diff
...
memory-backfill-seed: ## Lancer le seed v0.3 d'un repo prioritaire (usage: make memory-backfill-seed REPO=VPAI)
	...
	@case "$(REPO)" in \
		VPAI) EXTRA_ARGS='--path /home/mobuone/VPAI/playbooks/stacks/site.yml --path /home/mobuone/VPAI/playbooks/hosts/workstation.yml --path /home/mobuone/VPAI/inventory/hosts.yml --path /home/mobuone/VPAI/roles/llamaindex-memory-worker/defaults/main.yml --path /home/mobuone/VPAI/roles/llamaindex-memory-worker/tasks/main.yml --path /home/mobuone/VPAI/scripts/n8n-workflows/memory-run-report-ingest.json --path /home/mobuone/VPAI/scripts/n8n-workflows/memory-healthcheck.json --path /home/mobuone/VPAI/docs/runbooks/AI-MEMORY-OPERATIONS.md --path /home/mobuone/VPAI/Makefile' ;; \
...
deploy-obsidian: ## Deployer CouchDB Obsidian LiveSync sur Seko-VPN
	...
	$(ANSIBLE_PLAYBOOK) playbooks/utils/obsidian.yml --diff
...
deploy-obsidian-collectors: ## Deployer les collectors Obsidian (Sese-AI + Pi)
	...
	$(ANSIBLE_PLAYBOOK) playbooks/stacks/site.yml --tags obsidian-collector -e "target_env=prod" --diff
	$(ANSIBLE_PLAYBOOK) playbooks/hosts/workstation.yml --tags obsidian-collector-pi --diff
...
provision-hetzner: ## Provisionner un serveur Hetzner CX22
	...
	$(ANSIBLE_PLAYBOOK) playbooks/bootstrap/provision-hetzner.yml --diff

.PHONY: deploy-app-prod
deploy-app-prod: lint ## Deployer sur le serveur Prod Apps
	@echo "$(RED)>>> PROD APPS DEPLOYMENT$(NC)"
	$(ANSIBLE_PLAYBOOK) playbooks/hosts/app-prod.yml \
