...
(`44d60c86-2885-401e-9787-11a2d72df1e3`) — la version EXÉCUTÉE n'a pas changé
(seul le fait vérifié ; `workflow_history` compte 2 lignes au total pour ce
workflow, sans baseline pré-deploy pour trancher si le PUT en a ajouté une
...
~18h30, heure du contrôle). **Critère « exécution verte post-deploy » CLOS
2026-07-18 19:05** : exécution `33889 | success | 2026-07-18 19:00:00+02`
(première exécution cron postérieure au redeploy de 18h25, requête §5).
...
ce sont des preuves de green-run PRÉ-déploiement, pas post-déploiement. La
transfert de preuve est légitime ici (redeploy no-op, `activeVersionId`
inchangé = c'est exactement la version qui a tourné verte à l'heure qui reste
active) mais reste une inférence, pas une exécution verte observée
après coup. Aucun déclenchement manuel n'a été forcé pour combler ce point
(le cutover du 2026-07-18 a montré qu'un trigger réel non maîtrisé peut
produire une alerte Telegram effective — même classe de risque que
l'incident staging §Incident du runbook Sidecar). Rejouer la vérification
après le prochain tick horaire pour clore formellement ce critère :
```sql
SELECT id, status, "startedAt" FROM execution_entity
WHERE "workflowId" = 'NZZ9Ke6DXJTlkasa' AND "startedAt" > '2026-07-18 18:25:54'
