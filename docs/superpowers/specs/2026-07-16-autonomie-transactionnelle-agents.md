...
| **Couche paiement (funding borné)** — outils user = **Revolut PARTICULIER + Wise** (PAS Business, PAS Stripe) | ⚠️ **Pas d'API d'émission carte sur compte particulier** (Business API = pro only) → l'auto-émission par transaction n'est PAS faisable. **Chemin v1 = funding borné + HITL manuel** : **poche Revolut dédiée « agent » à petit solde** + sa carte virtuelle stockée dans **Vaultwarden**, l'Exécuteur (Skyvern) remplit le checkout. **I2 = le solde borné** (agent compromis ≤ solde ; recharge via HITL). Cartes **jetables** générées **à la main par l'humain au gate HITL** pour marchand risqué ponctuel. **Automatisation de l'émission = DIFFÉRÉE** jusqu'à Revolut/Stripe **Business** ou provider agent-natif (Agentcard.sh, à évaluer). Wise = transferts/payouts à côté. | poche Revolut particulier + carte dans Vaultwarden ; émission auto = future |
