...
| VE.7 | **Coffre agent — Tier 1** (tokens renouvelables, usage quotidien ; contrat vérifié : `docs/reference/VERIF-COFFRE-AGENTS-2026-06-10.md`) : (a) CLI **`rbw`** (Rust, hors npm — réponse à la compromission `@bitwarden/cli` 2026.4.0 vérifiée) sur waza+Sese, collection `infra-agents`, session via keyring/systemd-creds, jamais en clair disque ; (b) wrapper **`secret-run <ref> -- cmd`** : injecte en env du process enfant (jamais argv), **stdout/stderr filtrés des occurrences du secret** (pattern passwd-mcp vérifié), exit≠0 si ref absente, audit JSON (qui/quoi/quand, jamais la valeur) → Loki ; (c) **politique YAML** `secret → {commande, hôte} autorisés` dans le wrapper (épinglage anti prompt-injection) ; (d) deny-list settings (`Read(.env*)`, `rbw get`, `bw get`, `ansible-vault view` → hook exit 2 vers secret-run) + hook PostToolUse **redaction** regex (sk-/ghp_/hf_/eyJ/tskey-/AKIA) sur tous tool results ; (e) jobs à secrets exécutés sous le **user systemd du worker** (≠ user agent, `/proc` illisible cross-user) — l'agent déclenche, n'instrumente pas ; (f) **canary token** Vaultwarden jamais utilisé légitimement → alerte s'il sort ; (g) lookup Ansible + `no_log` partout ; secrets runtime (HF, RUNPOD, QDRANT, PAT, Headscale) hors `.env` clair | `pod-ingest.env` & co. sans secret en clair ; `secret-run` opérationnel + politique active ; rotation = MAJ Vaultwarden seule ; canary monitoré |
...
| VE.10 | **Coffre agent — Tier 2** (résolveur isolé, garantie forte — après VE.7) : conteneur dédié sur **Seko-VPN** (hôte Vaultwarden, aucun agent n'y a de shell — la "frontière shell" est physique) ; `bw serve` 127.0.0.1 (npm **pinné versions.yml + checksum**, jamais auto-update — compromission 2026.4.0) ; exposé via Tailscale en **actions uniquement** (jamais de `get`) : `run_authenticated_call(ref, endpoint)`, plus tard `secure_fill(ref, selector)` pour les logins web R2 Playwright (inspiré des 6 couches Cerberus : allowlist coffre, vérif URL avant injection, token par agent, rate-limit, audit sans valeurs, HITL) ; re-tokenisation des snapshots retour. Réservé : secrets forts, logins web, toute future donnée non renouvelable (carte réelle = jamais — virtuelle plafonnée) | une action authentifiée web/API exécutée de bout en bout sans que la valeur n'apparaisse dans aucun tool result (test : grep transcript) |
...
| 4.4 | 🔒 Scrubber credentials JSONL (préalable absolu à 4.5) — spec `.planning/notes/security-jsonl-credentials-leak.md` ; moteur candidat : **Presidio** (opérateur `encrypt` = seul réversible, vérifié — `docs/reference/VERIF-COFFRE-AGENTS-2026-06-10.md` §2.5) en plus des regex | test : JSONL avec secrets plantés → 100% scrubés |
