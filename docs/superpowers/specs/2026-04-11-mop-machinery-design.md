...
- **Auth :** SMTP magic link uniquement. `ENCRYPTION_SECRET` est la seule clé de session utilisée par Typebot 3.16.1. **Piège connu** : plusieurs issues GitHub (#1279, #942) sur la fiabilité SMTP en self-hosted. Mitigation : tester dès le premier déploiement avec un SMTP externe fiable (Sendgrid, Mailjet ou Postmark) ; fallback si SMTP KO = insertion directe en base PG (tables `User` + `Workspace` + `MemberInWorkspace` via Prisma studio ou SQL manuel).
