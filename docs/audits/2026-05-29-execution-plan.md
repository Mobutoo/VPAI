...
**Résidu EROFS — RÉSOLU 2026-05-30 ~01:12 via Option B (archi pérenne)** : OpenClaw 5.27 écrit des fichiers self-management sous `system/` (`openclaw.json.last-good`, `logs/config-health.json`, `logs/stability/*`, sentinels `.fs-safe-replace.*`, `plugin-skills/`) que le mount `system/:ro` (anti-drift, REX-Integration) bloquait → EROFS spam (non-fatal). **Fix** : `system/` passé en **rw** + `openclaw.json` monté **`:ro` seul** (single-file) → le moteur gère tout sauf le fichier de config, qui reste Ansible-owned/immuable (anti-drift préservé, scopé). Le handler `recreate: always` re-résout l'inode du mount single-file après chaque re-render. Pré-création `system/plugin-skills` ajoutée au rôle. **Vérifié** : EROFS=0, `openclaw.json` write→`Read-only` (immuable), `system/logs` write OK, `config-health.json`+`last-good` écrits par le moteur, conteneur `healthy`. Fichiers : `docker-stack/templates/docker-compose.yml.j2` (mount), `roles/openclaw/tasks/main.yml` (plugin-skills).
